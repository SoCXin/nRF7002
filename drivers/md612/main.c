...
void platform_init()
{
    ...
    // *** LOGS (could not use same UART for NRF_LOGS and custom logs simultaneously) // TODO: fix?
...
static void read_from_mpl(void)
{
    ...
    if (hal.report & PRINT_LINEAR_ACCEL) {
        if (inv_get_sensor_type_linear_acceleration(float_data, &accuracy, (inv_time_t*)&timestamp)) {
        	MPL_LOGI("Linear Accel: "NRF_LOG_FLOAT_MARKER" "NRF_LOG_FLOAT_MARKER" "NRF_LOG_FLOAT_MARKER"\r\n",
        			NRF_LOG_FLOAT(float_data[0]),
                    NRF_LOG_FLOAT(float_data[1]),
    ...
    if (hal.report & PRINT_GRAVITY_VECTOR) {
            if (inv_get_sensor_type_gravity(float_data, &accuracy,
                (inv_time_t*)&timestamp))
            	MPL_LOGI("Gravity Vector: "NRF_LOG_FLOAT_MARKER" "NRF_LOG_FLOAT_MARKER" "NRF_LOG_FLOAT_MARKER"\r\n",
            			NRF_LOG_FLOAT(float_data[0]),
                        NRF_LOG_FLOAT(float_data[1]),
                        NRF_LOG_FLOAT(float_data[2]));
    }
...
void send_status_compass() {
	...
	MPL_LOGI("Compass: "NRF_LOG_FLOAT_MARKER" "NRF_LOG_FLOAT_MARKER" "NRF_LOG_FLOAT_MARKER" ",
			NRF_LOG_FLOAT(data[0]/65536.f),
            NRF_LOG_FLOAT(data[1]/65536.f),
            NRF_LOG_FLOAT(data[2]/65536.f));
	MPL_LOGI("Accuracy= %d\r\n", accuracy);
...
static inline void run_self_test(void)
{
    ...
    if (result == 0x7) {
		MPL_LOGI("Passed!\n");
        MPL_LOGI("accel: "NRF_LOG_FLOAT_MARKER" "NRF_LOG_FLOAT_MARKER" "NRF_LOG_FLOAT_MARKER"\n",
                    NRF_LOG_FLOAT(accel[0]/65536.f),
                    NRF_LOG_FLOAT(accel[1]/65536.f),
                    NRF_LOG_FLOAT(accel[2]/65536.f));
        MPL_LOGI("gyro: "NRF_LOG_FLOAT_MARKER" "NRF_LOG_FLOAT_MARKER" "NRF_LOG_FLOAT_MARKER"\n",
...
static void handle_input(char c)
{
     ...
     */
    case 'd': // NOTE: need to increase UARD rate and UART_TX_BUF_SIZE to prevent hanging. 115200 + 4096, or 230400 + 2048
        mpu_reg_dump();
        break;
 ...
 * function.
 */
static void gyro_data_ready_cb(nrf_drv_gpiote_pin_t pin, nrf_gpiote_polarity_t action)
{
    hal.new_gyro = 1;
}
/*******************************************************************************/
...
{ 
  inv_error_t result;
    ...
    while(1){
        ...
        get_ms(&timestamp);//MPL_LOGI("Timestamp: %d\n", timestamp);
